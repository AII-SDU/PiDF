...
def plot_mean_roc(
    ...
    for method in methods_to_plot:
        if method not in roc_results:
            continue

        r = roc_results[method]
        fpr = r["mean_fpr"]
        tpr = r["mean_tpr"]
        std = r["std_tpr"]

        label = f"{method} (AUC={r['auc_mean']:.3f}$\\pm${r['auc_std']:.3f})"

        plt.plot(fpr, tpr, linewidth=1.8, label=label)
        plt.fill_between(
            fpr,
            np.maximum(tpr - std, 0),
            np.minimum(tpr + std, 1),
            alpha=0.12,
        )
...
def main():
    ...
    parser.add_argument("--outdir", type=str, default="runs/roc_254")
    ...
    # 保存数值结果
    summary_rows = []
    for method, r in roc_results.items():
        summary_rows.append({
            "method": method,
    ...
    pd.DataFrame(summary_rows).to_csv(
        os.path.join(args.outdir, "mean_roc_summary.csv"),
        index=False,
    )
